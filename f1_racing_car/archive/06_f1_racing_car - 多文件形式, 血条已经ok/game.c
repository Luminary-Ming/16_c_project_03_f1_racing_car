...
static void handle_events(void)
{
    ...
    while (SDL_PollEvent(&event))
    {
        ...
        else if (event.type == SDL_KEYDOWN)
        {
            ...
            else if (event.key.keysym.sym == SDLK_m && map_transition.state == TRANSITION_NONE && !game_over)
            {
            ...
            else if (event.key.keysym.sym == SDLK_c && !game_over)
            {
                current_car = (current_car + 1) % CARS_COUNT;
                current_car_texture = car_textures[current_car];
            }
            // 添加音乐控制功能
            else if (event.key.keysym.sym == SDLK_p && !game_over)
            {
                if (bgm)
                {
                    if (Mix_PausedMusic())
            ...
            else if (event.key.keysym.sym == SDLK_n && !game_over)  // 切换下一首
            {
                // 音乐文件数组
                const char *bgms[] = {
                    "assets/bgm/Insomnia - Craig David.mp3",
            ...
            else if (event.key.keysym.sym == SDLK_b && !game_over)  // 切换上一首
            {
                // 音乐文件数组
                const char *bgms[] = {
                    "assets/bgm/Insomnia - Craig David.mp3",
