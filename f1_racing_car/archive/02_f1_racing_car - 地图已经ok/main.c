...
int main(void)
{
	...
	int current_map = 0;
	SDL_Texture *map_textures[MAX_MAPS] = { NULL };  // 初始化所有地图纹理
	int map_heights[MAX_MAPS] = { 0 };  // 初始化所有地图的高度
	...
	if (!carSurface)
	{
		...
		for (int i = 0; i < map_total; i++)
			if (map_textures[i]) SDL_DestroyTexture(map_textures[i]);
	...
	while (running)  // 事件循环
	{
		...
		while (SDL_PollEvent(&event))
		{
			...
			else if (event.type == SDL_KEYDOWN)  // 键盘按键按下
			{
				...
				// 赛车边界检查
				if (car_x < 0) car_x = 0;
				if (car_x > WINDOW_W - CAR_WIDTH) car_x = WINDOW_W - CAR_WIDTH;
