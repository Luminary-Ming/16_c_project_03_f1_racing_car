...
#define ENEMY_CARS_COUNT 7  // 敌方车辆种类数量
...
int main(void)
{
	...
	for (int i = 0; i < MAX_ENEMIES; i++)
	{
		...
		enemies[i].speed = 10.0f + (rand() % 20) / 2.0f;  // 统一为 10-20
		enemies[i].active = 0;  // 初始不激活
		...
		if (enemy_spawn_timer >= enemy_spawn_delay)
		{
			...
			for (int i = 0; i < MAX_ENEMIES; i++)
			{
				if (!enemies[i].active)
				{
					...
					// 随机速度 - 增加基础速度
					enemies[i].speed = 10.0f + (rand() % 20) / 2.0f;  // 10-20
					...
					// 随机生成间隔
					enemy_spawn_delay = 40 + (rand() % 40);  // 40-80帧
		...
		for (int i = 0; i < MAX_ENEMIES; i++)
		{
			if (enemies[i].active)
			{
				...
				if (playerRect.x < enemyRect.x + enemyRect.w &&
					...
					playerRect.y + playerRect.w > enemyRect.h)
				{
					printf("碰撞！游戏结束！\n");
				}
			}
